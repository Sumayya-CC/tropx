...
- Rate limiting on the public-create job-queue collections (`passwordResetRequests`, `contactInquiries`, `accessRequests`) gates the trigger's outbound email, not the Firestore write — Firestore rules can't see IP/velocity, so `firestore.rules` stays `allow create: if true` for these and the limiting logic lives in `isRateLimited(scope, email)` in `index.ts`, keyed by a hashed email in `rateLimitCounters/{hash}`, config-driven via `settings/rateLimits` (`??` fallback to hardcoded defaults, same pattern as `isNotificationEnabled`). Follow this pattern — trigger-level gate, not a rules-level one — for any future public-create collection that needs rate limiting.
