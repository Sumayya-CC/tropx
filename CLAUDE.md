...
- **`coupons/{code}` uses the normalized code itself as the doc ID** — free uniqueness, O(1) lookup, no query needed. Trade-off accepted: a code can't be renamed (redemption history is keyed under it) — admins deactivate and create a new one instead.
- **Coupon release is two distinct functions, not one** (`releaseCoupons` for an incremental removal mid-edit vs `releaseAllCoupons` for the terminal cancel/full-return release) — chosen because conflating them either leaks a redemption slot forever (terminal flag reused on an incremental edit) or permanently locks an order out of further redemption (an incremental release setting the terminal flag). Rejected: a single `releaseCoupons` call for both cases.
...
Gen2, `firebase-functions/v2`. Split by domain (Prompt 5, 2026-08-04) — `index.ts` is now a thin re-export aggregator (`export * from "./domains/<name>"`); every actual trigger/callable lives in `functions/src/domains/*.ts` (`reconciliation.ts`, `shop-health.ts`, `auth-lifecycle.ts`, `notifications.ts`, `purchasing.ts`, `popular-products.ts`, `orders.ts`, `field-ops-transactions.ts`, `coupons.ts`), with shared infra in `core.ts` (bootstrap: `admin.initializeApp()`/`db`/secrets/`STAFF_ROLES`), `rate-limit.ts`, `email-templates.ts`, `staff-transactions-shared.ts` (the staff `actionBy` builder and order/return-sequence/totals helpers shared across the transactional `onCall` group), and `coupon-shared.ts` (peer to `staff-transactions-shared.ts` — coupon validate/redeem/release logic shared across `placeOrder`, `createAdminOrder`, `updateAdminOrder`, `saveOrderQuantityEdits`, `cancelOrder`/`cancelAdminOrder`, `approveReturn`, and `validateCoupon`). Import direction is one-way: `core.ts` imports nothing local; `rate-limit.ts`/`email-templates.ts`/`staff-transactions-shared.ts`/`coupon-shared.ts` import only `core.ts`; `domains/*.ts` import from the shared files but never from each other. `functions/src/function-contract.spec.ts` snapshots every exported function's real trigger/region/secret contract (read from its own `__endpoint` metadata) — any future reorganization must keep that snapshot byte-identical, since Firebase deletes-and-recreates a function whose export name or trigger config changes. Key patterns:
...
- **Coupons**: `coupons/{code}` CRUD is a plain staff Firestore write from the admin UI (like `products`) — no Cloud Function needed there. `functions/src/coupon-shared.ts` owns validate/redeem/release, called from inside every order-transaction function so a coupon can never be applied outside a re-validated server transaction. `validateCoupon` (onCall) is a read-only, rate-limited preview for cart UIs; `recomputeCouponUsage` (onCall, staff-only) is an on-demand drift-safety sweep with no scheduled twin — unlike the health/pipeline/reconciliation stamps, nothing runs it nightly.
- Other scheduled/on-call jobs follow similar naming: `nightly*`/`weekly*` for scheduled sweeps, `on*` for Firestore triggers, plain camelCase `onCall` functions for client-invoked RPCs (e.g. `placeOrder`, `refreshShopHealthNow`, `validateCoupon`).
- The active Firestore **database ID** (`tropx-dev` vs `tropx-prod`) is resolved at runtime from `GCLOUD_PROJECT`, not from environment config — see `core.ts`.
...
- `coupons/{code}` (and its `redemptions/{customerId}` subcollection) are **staff-only read/write**, unlike `products`/`categories` which are public-read. A customer never reads a coupon doc directly — they only ever reach coupon data through `validateCoupon`/`placeOrder` (Admin SDK, bypasses rules), the same "customer client never trusted with money-affecting data" precedent as `placeOrder`/`cancelOrder`/`submitReturn`.
- `returns/{returnId}`: `create` is `if false` for everyone, staff included — creation only ever happens server-side through `submitReturn` (Admin SDK, bypasses rules). This used to allow a staff `create` directly, which is exactly how the admin "create return" modal got away with writing return docs from the client for as long as it did (2026-08-18, see the `submitReturn` consolidation note below) — a reminder that a permissive rule on a collection a Cloud Function also owns is an invitation for some other code path to use the permissive rule instead of the function. `read` stays role-scoped (staff full, customer own-`customerId` only) and `update` stays staff-only (the admin reject-return flow still writes directly — a known, separately-tracked looseness, not this one). This leaves `returns` in a deliberate but asymmetric state: `create` is fully server-owned, `update` isn't. Defensible today (reject is status-only, not money-affecting, unlike create's amount/stock/coupon-release consequences) — but the end state ("returns fully server-owned, no client writes at all, matching orders/coupons") is a tracked open endpoint, not something this fix silently closed. Don't assume it's done just because `create` is locked.
...
- **Tax**: single 13% HST, added at checkout on the pre-discount-adjusted subtotal (`tax = (subtotal − discount) × rate`). Prices shown pre-tax; HST added at the total. `discount` is now `manualDiscountCents + couponDiscountCents` (order model, backward-compatible: the pre-existing `discountCents` field is their sum) — HST must always be computed on the coupon-discounted subtotal, never the gross subtotal with a coupon subtracted after tax. This is load-bearing across every order-transaction function (`placeOrder`, `createAdminOrder`, `updateAdminOrder`, `saveOrderQuantityEdits`) plus the portal cart and admin order-form client-side previews, which must mirror the same formula.
- **Coupons** (`coupons/{code}`, `functions/src/coupon-shared.ts`): `type: 'percentage'|'fixed'`, validated against `active`/`startsAt`/`expiresAt`/`minOrderSubtotalCents`/`maxUsesTotal`/`maxUsesPerCustomer` inside the same transaction that redeems them — the whole code set is validated together and rejected together, never partially applied. A non-stackable coupon can't combine with any other coupon on the same order. Percentage coupons are re-priced against the new subtotal on every order edit (`updateAdminOrder`, `saveOrderQuantityEdits`), and redemption is released — incrementally on removal, terminally (once, guarded by `order.couponsReleasedAt`) on cancel or full return — see "Decisions We Made And Why" for why release is two functions, not one. `submitReturn`/`approveReturn` gained `isFullReturn` (sums prior + this return's quantities against order items) to know when a return should trigger the terminal release; this is new, minimal tracking on a previously-untracked area, not a fully solved invariant — a pending return later rejected via a direct client write can leave the stamp stale.
...
- **Return refunds are net-of-discount, plus-tax** (`submitReturn`, `functions/src/domains/orders.ts`) — not the gross line price. Per return, against the order's frozen snapshot: `effectiveDiscountCents = min(order.discountCents, order.subtotalCents)`; `discountShareCents = round(effectiveDiscountCents × returnGrossCents / order.subtotalCents)`; `returnNetCents = max(0, returnGrossCents − discountShareCents)`; `returnTaxCents = round(returnNetCents × order.taxRatePercent / 100)`; `amountCents = returnNetCents + returnTaxCents` — same discount-first-then-tax ordering as `computeOrderTotals`, just applied to the returned share. `approveReturn` nets this `amountCents` straight off `order.totalCents`/`balanceCents`; no separate tax logic lives there. Proportional shares can drift by a cent or two across multiple partial returns (independent roundings), so the return that completes the order (`isFullReturn`) skips the formula and true-ups to `order.totalCents` minus any still-pending prior returns' `amountCents`, so the order always zeroes out exactly. This does not touch `couponDiscountCents`/`appliedCoupons` — those stay frozen per the coupon design; only the amount netted off `totalCents`/`balanceCents` changes. Fixed 2026-08-18 (was previously refunding gross, see repo memory).
- **`submitReturn` is the single, server-owned path for creating a return** — both the portal (customer, self-service) and the admin "create return" modal (staff, on behalf of any customer) call the same `onCall`. Staff callers skip the ownership check (`order.customerId === linkedCustomerId`) that portal callers get, are stamped `source: 'admin'` + a `buildStaffActionBy` `createdBy` instead of `source: 'customer_portal'` + the customer's name, and may set staff-only `internalNotes`. This was **not always true**: until 2026-08-18 the admin modal wrote return docs directly via a client Firestore batch, which meant admin-created returns never computed `isFullReturn` and so never triggered `approveReturn`'s terminal coupon release on a full return — a real leak (coupon redemption slots silently never freed on admin-side returns), not just the parallel tax-bug this shared. Consolidating both callers onto `submitReturn` closed both the math bug and the write-surface/coupon-leak bug in the same change. If a return-creation code path ever again bypasses this callable, assume it has both problems until proven otherwise.
...
- Coupons: exact-cents stacking (multiple stackable coupons sum independently), stacking rejection (non-stackable + anything else), usage-limit enforcement (per-customer/global/expiry/min-order), the tax-base-ordering assertion (HST computed after the coupon discount, not before), and the release-idempotency scenarios (remove→re-add→cancel releases exactly once; redeeming onto an already-terminally-released order is rejected).
...
- Namespace-style `admin.firestore.Timestamp`/`admin.firestore.FieldValue` static access crashes in the Functions Emulator when combined with a named-database `db.settings()` call (production on the real Cloud Functions runtime is unaffected — emulator-only). First found and fixed on `placeOrder`'s `FieldValue` usage; recurred at a second call site (`rate-limit.ts`'s `admin.firestore.Timestamp.fromMillis`, hit while adding `validateCoupon`'s rate limit, never previously exercised through a real `onCall` path in the emulator). Fixed the same way both times: use the modular `firebase-admin/firestore` import (`Timestamp.fromMillis`/`FieldValue.serverTimestamp`), not the namespace form. Treat this as a standing gotcha for any new `admin.firestore.*` static call, not a one-off.
