...
| Item | Status | Detail |
|---|---|---|
| Automated test coverage | Built | The testing philosophy documented in §10.7 (exact-cent money assertions, stock-clamp invariants, dual-run idempotency checks against the Firebase emulators, per-role rules testing) now has a real suite behind it — 22 `functions/src/*.spec.ts` files (243 tests as of 2026-08-06, incl. `firestore-rules.spec.ts`/`storage-rules.spec.ts`) plus 7 frontend `*.spec.ts` files. Extending coverage to newer surfaces as they land is ongoing, not a from-scratch buildout |
| CI/CD pipeline | Built | `.github/workflows/ci.yml` runs lint/typecheck/build/test for both the Angular app and Cloud Functions on every PR and non-master push (verification only, no deploy); `deploy-preview.yml` and `doc-drift.yml` cover PR previews and doc-drift checking respectively. Deploys to dev/prod remain the documented manual command sequence — this row was stale, written before Prompt 1 (CI/CD foundation) landed |
...
| Cloud Functions modularization | Built | Done as Prompt 5 (phases 5.0-5.10): `functions/src/index.ts` went from a single 7,700-line file to a 67-line re-export aggregator, with every trigger/callable split into `functions/src/domains/*.ts` by business domain and shared infra extracted to `core.ts`/`rate-limit.ts`/`email-templates.ts`/`staff-transactions-shared.ts`. See §6 and ADR-016. This row was stale, written before that split landed |
