...
| `returns` | `returnNumber` (`RET-YYYY-NNNN`), `type` (`credit_note`\|`refund`), `status` (`pending`\|`approved`\|`rejected`), `source` (`admin`\|`customer_portal`\|`cancellation`), `stockRestored`, `amountCents` (net-of-discount, plus-tax — proportional share of the order's frozen `discountCents`/`taxRatePercent`, true-up'd to `totalCents` on the completing return), `isFullReturn` (sums prior + this return's quantities against order items — new, minimal tracking; a pending return later rejected via a direct client write can leave it stale) | No financial impact until `approved`; `isFullReturn` gates the terminal coupon release on `approveReturn`. `create` is server-only (`submitReturn`, both customer and staff callers) — `firestore.rules` denies client `create` entirely |
